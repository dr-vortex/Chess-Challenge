...
namespace Frederox.Negamax
{
    public class Negamax : IChessBot
    {
        ...
        public Move Think(Board board, Timer timer)
        {
            ...
            int score = NegamaxEval(board, mDepth, negativeInfinity, -negativeInfinity, board.IsWhiteToMove);
        ...
        int NegamaxEval(Board board, byte depth, int alpha, int beta, bool asWhite)
        {
            int alphaOriginal = alpha;
            TTEntry? ttEntry = getTransposition(board.ZobristKey);
            ...
            {
                board.MakeMove(move);
                int moveScore = Math.Max(value, -NegamaxEval(board, (byte)(depth - 1), -beta, -alpha, !asWhite));
                board.UndoMove(move);
        ...
        TTEntry? getTransposition(ulong zobristKey)
        {
            TTEntry? entry = transpositions[(int)(zobristKey % 2080895)];
        ...
        {
            transpositions[(int)(zobristKey % 2396745)] = new TTEntry
            {
